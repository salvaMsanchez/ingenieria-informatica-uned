...
FROM Reservas
WHERE FechaEntrada BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '30' DAY
...
SELECT R.CodHotel,
       ...
       SUM(R.NumHabitac * R.NumNoches * R.PrecioNochHab) AS IngresosFuturos
FROM Reservas R
JOIN Hoteles H ON R.CodHotel = H.CodHotel
WHERE R.FechaEntrada > CURRENT_DATE
...
WHERE EXTRACT(MONTH FROM R.FechaEntrada) = EXTRACT(MONTH FROM H.FecInaug);
...
SELECT H.Nombre AS NombreHotel,
       EXTRACT(YEAR FROM R.FechaEntrada) AS Año,
       EXTRACT(MONTH FROM R.FechaEntrada) AS Mes,
       (COUNT(*) * 100.0 / MAX(H.NumHabitac)) AS PorcentajeOcupacion
FROM Reservas R
JOIN Hoteles H ON R.CodHotel = H.CodHotel
WHERE EXTRACT(YEAR FROM R.FechaEntrada) = 2023
GROUP BY H.Nombre, EXTRACT(YEAR FROM R.FechaEntrada), EXTRACT(MONTH FROM R.FechaEntrada)
HAVING (COUNT(*) * 100.0 / MAX(H.NumHabitac)) > 2;
...
SELECT NIF, Nombre, Apellido1, Apellido2, FecNacim, Direccion, Ciudad
FROM Clientes
WHERE NIF NOT IN (
    SELECT DISTINCT R.NIF
    FROM Reservas R
    WHERE R.FechaReserva >= CURRENT_DATE - INTERVAL '1 year'
);
...
SELECT Nombre, Categoria, AVG(PrecioNochHab) AS PrecioMedioHabitacion
FROM Hoteles
JOIN Reservas ON Hoteles.CodHotel = Reservas.CodHotel
...
CREATE OR REPLACE FUNCTION CostoReserva(p_cod_reserva VARCHAR(10))
RETURNS NUMERIC AS
$$
DECLARE
    v_costo NUMERIC;
BEGIN
    SELECT (NumNoches * NumHabitac * PrecioNochHab)
    INTO v_costo
    FROM Reservas
    WHERE CodReserva = p_cod_reserva;
...
# Llamada a la función `CostoReserva()` pasándole la reserva "R001" a modo de ejemplo

SELECT CostoReserva('R001');
...
SELECT 
    C.Nombre AS NombreCliente,
    COUNT(R.CodReserva) AS NumReservas,
    AVG(CostoReserva(R.CodReserva)) AS CostoMedioReservas,
    SUM(CostoReserva(R.CodReserva)) AS CostoTotalReservas
FROM Clientes C
LEFT JOIN Reservas R ON C.NIF = R.NIF
GROUP BY C.Nombre;
